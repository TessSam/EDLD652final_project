...
library(ez)
library(effects)
library(grDevices) # RG: added this since it couldn't find the `quartz()` function but then it still didn't work, I suspect b/c I have windows OS
```
...
# RG: to make this easier to plot, i would use pivot_wider()

acc_ind_wide <- acc_ind %>% 
  pivot_wider(names_from = CUETYPE, values_from = ACC)

# RG: now you can easily use ggplot to plot

acc_ind_wide %>% 
  ggplot(aes(valid, invalid)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", color = "red", size = 2) + # RG: totally optional, just added it for fun, lol
  labs(title = "Accuracy for Valid Versus Invalid Cues", # RG: added placeholder labels that you can edit
       x = "Accuracy Among Valid Cues",
       y = "Accuracy Among Invalid Cues") +
  theme_light()
  
# RG: now this is a good base plot that you can customize as you'd like to add your own flair. i did a couple of simple aesthetic things to match your other plots. and maybe you could use ggtext() to add an annotation that shows the correlation between valid and invalid cues
...
#grDevices::quartz(width=5,height=4) #RG: i think this code is not reproducible for people with Windows OS
...
plot2 <- ggplot(ds_p, aes(x=EVnum, y=Estimate,group=cond,color=cond)) + 
  ...
  ylab("Decision Weight")+xlab("Element Position")+
  labs(title = "placeholder title") + # RG: added this
  theme(plot.title = element_text(size =20,face='bold'))+
  theme(legend.key = element_blank())+
  #theme(legend.position="none")+
  theme(legend.position = c(0.45,0.9), # RG: all this can be done at the same time
        ...
        axis.title=element_text(family="Helvetica", face="bold",vjust=0.8))

# RG: i don't think you need any of these strip arguments bc you aren't using facet_wrap in this plot
#         strip.text=element_text(family="Helvetica", face="bold",vjust=0.4,size=10),
#         strip.text=element_text(family="Helvetica", face="bold",vjust=0.4,size=12),
...
plot3 <- ggplot(acc_ind_wide,aes(x=valid,y=invalid,size=1))+
...
```{r}
# i haven't really changed almost anything about the plot itself in this code, but i just mostly copy/pasted and edited the structure of the code to enhance readability and understanding. so much of this is just stylistic preference so it's very open to everyone's own preferences

acc_ind_wide %>% 
  ggplot(aes(x = valid, y = invalid, size, size = 1)) +
  geom_point(size = 5, color = "black") +
  geom_smooth(method = lm, se = F, fullrange = T, 
              alpha = 0.2, size = 2, color = "red") +
  labs(title = paste0("Correlations of Cueing Congruency, 6 Items: ", r1, ", ", r2), 
       # RG: i would maybe put the "6 items..." in a subtitle if possible, but this looks good too! and i slightly changed the title here to fix some spacing and missing comma issues
       y = "Invalid Cueing",
       x = "Valid Cueing") +
  scale_x_continuous(limits = c(0.5, 1), breaks = seq(0.5, 1, 0.1)) +
  scale_y_continuous(limits = c(0,0.72), breaks = seq(0, 1, 0.1)) +
  theme(legend.key = element_blank(),
        ...
        plot.title = element_text(family = "Helvetica", face = "bold", 
                                size = 16, vjust = 1.4, hjust = 1),
        axis.title = element_text(family = "Helvetica", face = "bold", vjust = 0.8))

# alternative code with subtitle
...
acc_ind_wide %>% 
  ...
  geom_smooth(method = lm, se = F, fullrange = T, 
              alpha = 0.2, size = 2, color = "red") +
  labs(title = "Correlations of Cueing Congruency", 
       subtitle = paste0("6 Items: ", r1, ", ", r2), # or you could also put this in a caption. not sure which i'd prefer
       y = "Invalid Cueing",
       x = "Valid Cueing") +
  scale_x_continuous(limits = c(0.5, 1), breaks = seq(0.5, 1, 0.1)) +
  scale_y_continuous(limits = c(0,0.72), breaks = seq(0, 1, 0.1)) +
  theme(legend.key = element_blank(),
        ...
        plot.subtitle = element_text(family = "Helvetica", face = "bold", size = 14),
        axis.title = element_text(family = "Helvetica", face = "bold", vjust = 0.8))

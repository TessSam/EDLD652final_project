...
#effects for each position separately for congruency(consistant or inconsistant) with cue category   
coefs<-c(coefs[2:5],coefs[2:5]+coefs[6:9]);
cond<-c(rep("incongruent",1,4),rep("congruent",1,4));

EVnum<-rep(seq(1,4,1),2);
#Empty vector to extracted stde
stde<-rep(NA,1,8);
#extract row 2 - 10, on column 2
stde<-m$coefficients[3:10,2]
